...
- (void)dealloc{
	...
	CY_RELEASE_SAFELY(_filterSettingsSlider);
	CY_RELEASE_SAFELY(_filterSelectScrollView);
	CY_RELEASE_SAFELY(filterView);
	[super dealloc];
}
...
- (void)selectFilter:(UIButton *)button{
	switch (button.tag) {
		case 1:{
			filterType = GPUIMAGE_SHARPEN;//锐化
		}break;
		case 2:{
			filterType = GPUIMAGE_SEPIA;
		}break;	
		case 3:{
			filterType = GPUIMAGE_CROSSHATCH;
		}break;
		case 4:{
