...
- (id)init
{
	...
	[self setRGBControlPoints:[NSArray arrayWithObjects:[NSValue valueWithCGPoint:CGPointMake(0.0, 0.0)], [NSValue valueWithCGPoint:CGPointMake(127.5, 127.5)], [NSValue valueWithCGPoint:CGPointMake(255, 255)], nil]];
...
//            point.y = point.y * 255;
		...
		for (int i=0; i<256; i++) 
		{
			if(i<[splinePoints count]){
				...
				[preparedSplinePoints addObject:[NSNumber numberWithFloat:newPoint.y]];  
			}else {
				[preparedSplinePoints addObject:[NSNumber numberWithFloat:255.0]]; 
			}
...
- (NSArray *)splineCurve:(NSArray *)points
{
	...
	for(int i=0; i<n-1 ; i++) 
	{
		...
		for(int x=cur.x;(int)x<next.x;x++) 
		{
...
- (void)updateToneCurveTexture
{
	...
	if ( ([_redCurve count] >= 256) && ([_greenCurve count] >= 256) && ([_blueCurve count] >= 256) )
	{
		for (unsigned int currentCurveIndex = 0; currentCurveIndex < 256; currentCurveIndex++)
		{
			...
			// NSLog(@" blue :%f" , [[_blueCurve objectAtIndex:currentCurveIndex] floatValue]);
			toneCurveByteArray[currentCurveIndex * 4] = 
			// currentCurveIndex +
			
			[[_blueCurve objectAtIndex:currentCurveIndex] floatValue];
			// NSLog(@" green :%f" , [[_greenCurve objectAtIndex:currentCurveIndex] floatValue]);
			
			toneCurveByteArray[currentCurveIndex * 4 + 1] =
			// currentCurveIndex + 
			[[_greenCurve objectAtIndex:currentCurveIndex] floatValue];
			
			// NSLog(@" red :%f" , [[_redCurve objectAtIndex:currentCurveIndex] floatValue]);
			toneCurveByteArray[currentCurveIndex * 4 + 2] = 
			// currentCurveIndex +
			[[_redCurve objectAtIndex:currentCurveIndex] floatValue];
